...
Academic Daily Scholar 是一个面向教育研究的自动化学术日报系统。它会在每天北京时间上午 8 点检索最近 24 小时内发表的英文期刊论文，按 SSCI 白名单、教育主题关键词和排除规则筛选，再调用 OpenAI 兼容 API 生成中文精读摘要，最后通过网易 163 SMTP 发送 HTML 邮件，并在本地保存 Markdown 与真正的 Word .docx 文件。
...
- SSCI 白名单：支持读取本地 .xlsx 白名单，按期刊名与 ISSN 匹配。
...
编辑 .env：
    ...
    SSCI_WHITELIST_PATH=data/SSCI期刊白名单_2026-06.xlsx
...
- off：不使用白名单。
- 白名单文件已经放入仓库 data/SSCI期刊白名单_2026-06.xlsx，GitHub Actions 可直接读取。
...
| MAIL_TO | agony2023@qq.com |
| SSCI_WHITELIST_PATH | 通常不需要设置，工作流已固定为 data/SSCI期刊白名单_2026-06.xlsx |
...
3. 执行 python main.py run-once。
4. 将生成的 daily/ 和 logs/ 提交回仓库。
